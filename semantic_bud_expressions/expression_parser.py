...
class budExpressionParser:
    ...
    def parse(self, expression: str) -> Node:
        ...
        #
        # bud-expression :=  ( alternation | optional | parameter | text )*
        #
        parse_bud_expression = self.parse_between(
            NodeType.EXPRESSION,
            TokenType.START_OF_LINE,
            TokenType.END_OF_LINE,
        ...
        tokens = budExpressionTokenizer().tokenize(expression)
        _, ast_node = parse_bud_expression(Parser(expression, tokens, 0))
        return ast_node
